...
/**
 * Class Hat_Bazar_General_Repeater
 */
class Hat_Bazar_General_Repeater extends WP_Customize_Control {
	 ...
	 * Control for image
	 *
	 * @var bool $hat_bazar_image_control Control for image
	 */
	 ...
	 * Control for icon
	 *
	 * @var bool $hat_bazar_icon_control Control for icon
	 */
	 ...
	 * Control for title
	 *
	 * @var bool $hat_bazar_title_control Control for title
	 */
	 ...
	 *
	 * @var bool $hat_bazar_subtitle_control Control for subtitle
	 */
	 ...
	 * Control for text
	 *
	 * @var bool $hat_bazar_text_control Control for text
	 */
	 ...
	 * Control for link
	 *
	 * @var bool $hat_bazar_link_control Control for link
	 */
	 ...
	 *
	 * @var bool $hat_bazar_shortcode_control Control for shortcode
	 */
	 ...
	 *
	 * @var bool $hat_bazar_socials_repeater_control Control for repeater
	 */
	private $hat_bazar_socials_repeater_control = false;

	/**
	 * Icon container.
	 *
	 * @var string $hat_bazar_icon_container Icon container.
	 */
	private $hat_bazar_icon_container = '';
	...
	public function __construct( $manager, $id, $args = array() ) {
		...
		$this->add_field_label = esc_html__( 'Add new field', 'hat-bazar' );
		if ( ! empty( $args['add_field_label'] ) ) {
			$this->add_field_label = $args['add_field_label'];
		}

		$this->boxtitle = esc_html__( 'Cusomizer Repeater', 'hat-bazar' );
		...
		if ( ! empty( $args['hat_bazar_socials_repeater_control'] ) ) {
			$this->hat_bazar_socials_repeater_control = $args['hat_bazar_socials_repeater_control'];
		}
		...
		if ( file_exists( get_template_directory() . '/inc/customizer-repeater/inc/icons.php' ) ) {
			$this->hat_bazar_icon_container = 'inc/customizer-repeater/inc/icons';
		}
	}
	...
	public function enqueue() {
		...
		wp_enqueue_style( 'hat-bazar-font-awesome', hat_bazar_get_file( '/css/font-awesome.min.css' ), '4.7' );

		wp_enqueue_style( 'hat-bazar-admin-stylesheet', hat_bazar_get_file( '/inc/customizer-repeater/css/admin-style.css' ), '1.0.0' );

		wp_enqueue_script( 'hat-bazar-script', hat_bazar_get_file( '/inc/customizer-repeater/js/customizer_repeater.js' ), array( 'jquery', 'jquery-ui-draggable' ), '1.0.1', true );

		wp_enqueue_script( 'hat-bazar-fontawesome-iconpicker', hat_bazar_get_file( '/inc/customizer-repeater/js/fontawesome-iconpicker.min.js' ), array( 'jquery' ), '1.0.0', true );

		wp_enqueue_style( 'hat-bazar-fontawesome-iconpicker-script', hat_bazar_get_file( '/inc/customizer-repeater/css/fontawesome-iconpicker.min.css' ) );
	}
	...
	public function render_content() {
		...
		<div class="customizer-repeater-general-control-repeater customizer-repeater-general-control-droppable">
			<?php
			if ( ! hat_bazar_general_repeater_is_empty( $values ) ) {
				...
				<div class="customizer-repeater-general-control-repeater-container customizer-repeater-draggable">
					...
					<div class="customizer-repeater-box-content-hidden">
						...
						if ( $this->hat_bazar_title_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Title', 'hat-bazar' ), $this->id, 'hat_bazar_title_control' ),
									'class' => 'customizer-repeater-title-control',
									'type'  => apply_filters( 'repeater_input_types_filter', '', $this->id, 'hat_bazar_title_control' ),
								), $title
							);
						}
						...
						if ( $this->hat_bazar_subtitle_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Subtitle', 'hat-bazar' ), $this->id, 'hat_bazar_subtitle_control' ),
									'class' => 'customizer-repeater-subtitle-control',
									'type'  => apply_filters( 'repeater_input_types_filter', '', $this->id, 'hat_bazar_subtitle_control' ),
								), $subtitle
							);
						}
						...
						if ( $this->hat_bazar_text_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Text', 'hat-bazar' ), $this->id, 'hat_bazar_text_control' ),
									'class' => 'customizer-repeater-text-control',
									'type'  => apply_filters( 'repeater_input_types_filter', 'textarea', $this->id, 'hat_bazar_text_control' ),
								), $text
							);
						}
						...
						if ( $this->hat_bazar_link_control ) {
							$this->input_control(
								array(
									'label'             => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Link', 'hat-bazar' ), $this->id, 'hat_bazar_link_control' ),
									'class'             => 'customizer-repeater-link-control',
									'sanitize_callback' => 'esc_url_raw',
									'type'              => apply_filters( 'repeater_input_types_filter', '', $this->id, 'hat_bazar_link_control' ),
								), $link
							);
						}
						...
						if ( $this->hat_bazar_shortcode_control == true ) {
							$this->input_control(
								array(
									'label' => apply_filters( 'repeater_input_labels_filter', esc_html__( 'Shortcode', 'hat-bazar' ), $this->id, 'hat_bazar_shortcode_control' ),
									'class' => 'customizer-repeater-shortcode-control',
									'type'  => apply_filters( 'repeater_input_types_filter', '', $this->id, 'hat_bazar_shortcode_control' ),
								), $shortcode
							);
						}

						if ( $this->hat_bazar_socials_repeater_control == true ) {
						...
						<button type="button" class="social-repeater-general-control-remove-field"
							...
							?>
						>
							<?php esc_html_e( 'Delete field', 'hat-bazar' ); ?>
						</button>
		...
		>
			...
			<span class="description customize-control-description">
				...
				printf( /* translators: %s is link to FontAwesome */
					esc_html__( 'Note: Some icons may not be displayed here. You can see the full list of icons at %s', 'hat-bazar' ),
					/* translators: %s is link label*/
					sprintf(
						'<a href="http://fontawesome.io/icons/" rel="nofollow">%s</a>',
						esc_html__( 'FontAwesome', 'hat-bazar' )
					)
				);
				?>
			</span>
			...
			<?php get_template_part( $this->hat_bazar_icon_container ); ?>
		</div>
		<?php
	}
	...
	private function image_control( $value = '', $show = '' ) {
		...
		>
			...
			<input type="button" class="button button-secondary customizer-repeater-custom-media-button" value="<?php esc_attr_e( 'Upload Image', 'hat-bazar' ); ?>" />
		</div>
		<?php
	}
	...
	private function repeater_control( $value = '' ) {
		...
		$show_del        = 0;
		?>
		<span class="customize-control-title"><?php esc_html_e( 'Social icons', 'hat-bazar' ); ?></span>
		<?php
		if ( ! empty( $value ) ) {
		...
		if ( ( count( $social_repeater ) == 1 && '' === $social_repeater[0] ) || empty( $social_repeater ) ) {
			...
			<div class="customizer-repeater-social-repeater-container">
				<div class="customizer-repeater-rc input-group icp-container">
					...
					echo '</div>';
					get_template_part( $this->hat_bazar_icon_container );
					echo '<input type="text" class="customizer-repeater-social-repeater-link" placeholder="' . esc_attr__( 'Link', 'hat-bazar' ) . '">';
					echo '<input type="hidden" class="customizer-repeater-social-repeater-id" value="">';
					?>
					<button class="social-repeater-remove-social-item" style="display:none">
						<?php esc_html_e( 'Remove Icon', 'hat-bazar' ); ?>
					</button>
				</div>
				<input type="hidden" id="social-repeater-socials-repeater-colector" class="social-repeater-socials-repeater-colector" value=""/>
			</div>
			<button class="social-repeater-add-social-item button-secondary"><?php esc_html_e( 'Add icon', 'hat-bazar' ); ?></button>
			<?php
		} else {
			?>
			<div class="customizer-repeater-social-repeater">
			...
			foreach ( $social_repeater as $social_icon ) {
				...
				get_template_part( $this->hat_bazar_icon_container );
				echo '<input type="text" class="customizer-repeater-social-repeater-link" placeholder="' . esc_html__( 'Link', 'hat-bazar' ) . '" value="';
				...
				echo '">' . esc_html__( 'Remove Icon', 'hat-bazar' ) . '</button>';
				echo '</div>';
			}
