...
jQuery( document ).ready(
	function(){
		...
		if ( home_window_width_old < 750  ) {
			jQuery( '#our_services_wrap' ).hatbazargridpinterest( {columns: 1,selector: '.service-box'} );
			jQuery( '#happy_customers_wrap' ).hatbazargridpinterest( {columns: 1,selector: '.testimonials-box'} );
		} else {
			jQuery( '#our_services_wrap' ).hatbazargridpinterest( {columns: 3,selector: '.service-box'} );
			jQuery( '#happy_customers_wrap' ).hatbazargridpinterest( {columns: 3,selector: '.testimonials-box'} );
		}
	}
);
...
jQuery( document ).ready(
	function(){
		jQuery( 'html' ).click(
			function() {
				jQuery( '.hat_bazar_map_overlay' ).show();
			}
		);
		...
		jQuery( '.hat_bazar_map_overlay' ).on(
			'click',function(){
				jQuery( this ).hide();
			}
		);
...
jQuery( document ).ready(
	function(){
		if (jQuery( '.overlay-layer-nav' ).hasClass( 'sticky-navigation-open' )) {
			...
			$hat_bazar_header_height += 84;
			jQuery( '.header .overlay-layer' ).css( 'padding-top',$hat_bazar_header_height );
		}
